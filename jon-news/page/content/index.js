      ...
      return `<tr>
                <td>
                  <img src="${item.cover.type === 0 ? `
                    https://img2.baidu.com/it/u=2640406343,1419332367&amp;fm=253&amp;fmt=auto&amp;app=138&amp;f=JPEG?w=708&amp;h=500` : item.cover.images[0]}" alt="">
                </td>
                <td>${item.title}</td>
                <td>
...
async function setChannleList() {
  const result = await axios({
    ...
    // console.log(result);
    const htmlStr = `<option value="" selected="">请选择文章频道</option>` + result.data.channels.map(item =>
  ...
  if (e.target.classList.contains('del')) {
    ...
    const delId = e.target.parentNode.dataset.id
      ...
      if (children.length === 1 && queryObj.page !== 1) {
        queryObj.page--
        document.querySelector('.page-now').innerHTML = `第 ${queryObj.page}`
      }
